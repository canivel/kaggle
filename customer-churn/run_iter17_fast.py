"""Iter17 FAST: KDE + price fairness + contract tension (NO KNN - too slow)."""
import sys
...
def main():
    ...
    cfg_stats = train.groupby("_cfg")["MonthlyCharges"].agg(["mean","std"])
    for df in [train, test]:
        pm = df["_cfg"].map(cfg_stats["mean"])
        ps = df["_cfg"].map(cfg_stats["std"]).fillna(1).replace(0,1)
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(train, y)):
        ...
        X_va[cat_cols] = oe.transform(X_va[cat_cols]); X_te[cat_cols] = oe.transform(X_te[cat_cols])
        gm_f = y_tr.mean()
        ...
        kde_ch = KernelDensity(bandwidth=0.5).fit(Xn_tr[y_tr.values==1])
        kde_st = KernelDensity(bandwidth=0.5).fit(Xn_tr[y_tr.values==0])
        for Xn, df in [(Xn_tr, X_tr), (sc.transform(X_va[num_cols].fillna(0)), X_va), (sc.transform(X_te[num_cols].fillna(0)), X_te)]:
            df["kde_llr"] = (kde_ch.score_samples(Xn)-kde_st.score_samples(Xn)).astype("float32")
        # Ridge
        sc2 = StandardScaler()
        r = Ridge(alpha=10.0)
        r.fit(sc2.fit_transform(X_tr.fillna(0)), y_tr)
        X_tr["ridge"]=np.clip(r.predict(sc2.transform(X_tr.fillna(0))),0,1).astype("float32")
        X_va["ridge"]=np.clip(r.predict(sc2.transform(X_va.fillna(0))),0,1).astype("float32")
        X_te["ridge"]=np.clip(r.predict(sc2.transform(X_te.fillna(0))),0,1).astype("float32")
    ...
    generate_submission(test_ids, test_preds, "id", "Churn", "submissions/iter17_fast.csv")
