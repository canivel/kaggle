# =====================================================================
# FORGE v25 â€” BFS + MCTS v2 + Persistent CNN
#
# Over v24 (0.26 LB):
# FIX 1: MCTS budget per-level â€” L0 gets 90s, L1 gets 180s (2x RHAE weight),
#   L2+ gets 240s (3x weight). Previously all levels capped at 90s.
# FIX 2: Informed MCTS rollout â€” rollout uses productive actions (those that
#   changed frames in this game) ranked by global productivity count.
#   Pure random rollout often wastes 8/12 steps on no-ops.
# FIX 3: MCTS->CNN buffer seeding â€” when MCTS times out without solution,
#   all explored (state, action, reward) transitions are fed into the CNN
#   replay buffer. CNN starts warm instead of cold.
# FIX 4: BFS max_states raised 100K->200K â€” prevents premature timeout on
#   games with large solvable state spaces (tn36, sp80 L1).
# FIX 5: MCTS iter limit 2000->3000 â€” more search per time budget.
#
# Keeps all v24: ActionScanner dedup, persistent weights, beam search,
# cross-level transfer, affine BFS transfer, ACMD, IDDFS.
# =====================================================================
import copy
import glob
import hashlib
...
_CKPT_PATH = '/tmp/forge_v25_ckpt.pt'
...
# ==================== ACTION SCANNER (for MCTS â€” unique-effect dedup) ====================
...
class ActionScanner:
    ...
    Dedup reduces 5000 click positions â†’ 5-20 unique frame effects.
    """
...
class MCTSSolver:
    ...
    def _rollout(self, g, max_depth: int = 8) -> float:
        # FIX 2: use productive actions (frame-changing) for rollout â€” faster wins
    ...
    def solve(self) -> Optional[List]:
        ...
        # path_store: node_id â†’ action sequence to reach node
...
class MyAgent(Agent):
    ...
    def __init__(s, *a, **kw):
        ...
        s._mcts_step = 0
        s._mcts_solutions: Dict[int, List] = {}  # level â†’ solution
        s._mcts_seed_transitions: List = []       # FIX 3: transitions for CNN seeding
    ...
    def _try_mcts_solve(s, level_idx, game_cls, f0, bg):
        ...
        # FIX 1: Level-weighted MCTS budget â€” later levels worth more (RHAE weight l+1)
        # L0: 90s, L1: 180s (2x RHAE weight), L2+: 240s (3x weight)
        level_weight = min(level_idx + 1, 3)
        base_budget = 90 * level_weight
        mcts_budget = min(remaining * 0.15, base_budget)
        mcts_budget = max(20, mcts_budget)
